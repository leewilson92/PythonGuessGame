...
合并自用户最初的 6 步：开发与"写白盒测试"合并（TDD 同上下文）；"功能测试 + 性能测试"合并为 Mac 端验证；代码检视保持独立。

## 门禁（gates）

1. **无设计文档不开发**：`architect` 文档经人确认后才进 `developer`。
2. **有阻断项不进验证**：`reviewer` 列出 🔴 必须先由 `developer` 修复。
3. **不全绿不合并**：`verifier` 编译/单测/功能必须通过才提交或更新 PR。

## 如何调用

在本仓库内的 Claude Code 中，按阶段把任务交给对应子agent，例如：
- "用 architect 设计 X 功能" → 评审 `docs/design/X.md` → 确认。
